...
int main(int argc, char **argv)
{
    ...
    for (;;) {
		...
		for (int i = 1; i <= ny; i++) {
			for (int j = 1; j <= nx; j++) {
				int ind = IND(i, j);
				local_newgrid[ind] = (local_grid[IND(i - 1, j)] + 
	...
	if (rank == 0) {
		...
		printf("p=%d; rows=%d; cols=%d; ttotal=%.6f; tmpi=%.6f; allred=%.6f; thalo %.6f\n",
		commsize, rows, cols, prof[0], prof[1] + prof[2], prof[1], prof[2]);
    } else {
